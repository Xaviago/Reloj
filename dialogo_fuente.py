...
class DialogoFuente(tk.Toplevel):
    def __init__(self, parent, fuentes_disponibles, fuente_actual):
        super().__init__(parent)
        self.title("Cambiar fuente")
        self.fuentes_disponibles = fuentes_disponibles
        self.resultado = None

        tk.Label(self, text="Introduce el nombre de la fuente:").pack(padx=10, pady=5)
        self.entrada = tk.Entry(self)
        self.entrada.insert(0, fuente_actual)
    ...
    def actualizar_sugerencias(self, event=None):
        ...
        for fuente in self.fuentes_disponibles:
            if texto in fuente.lower():
                self.lista_sugerencias.insert(tk.END, fuente)
    ...
    def seleccionar_sugerencia(self, event=None):
        if self.lista_sugerencias.curselection():
            fuente = self.lista_sugerencias.get(self.lista_sugerencias.curselection())
            self.entrada.delete(0, tk.END)
            self.entrada.insert(0, fuente)
            self.aceptar()
    
    def aceptar(self):
        texto = self.entrada.get()
        for fuente in self.fuentes_disponibles:
            if fuente.lower() == texto.lower():
                texto = fuente
                break
